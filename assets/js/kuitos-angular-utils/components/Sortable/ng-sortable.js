...
(function (factory) {
	...
	angular.module('ng-sortable', [])
			...
			return {
				...
				link: function (scope, $el, attrs) {
					...
					function _emitEvent(/**Event*/evt, /*Mixed*/item) {
                        ...
                        // Bug: https://bugzilla.mozilla.org/show_bug.cgi?id=1153603
                        if (eventType === "endEvent") {
                            eventType = "end";
                        }

						var name = 'on' + eventType.charAt(0).toUpperCase() + eventType.substr(1);
					...
					sortable = Sortable.create(el, Object.keys(options).reduce(function (opts, name) {
                        ...
                        onDragOutRemove: function (/**Event*/evt) {
                            _emitEvent(evt, removed);
                        },
					...
					if (ngSortable && !/{|}/.test(ngSortable)) { // todo: ugly
						angular.forEach([
							...
							'onStart', 'onEnd', 'onAdd', 'onUpdate', 'onRemove', 'onDragOutRemove', 'onSort'
						], function (name) {
        ...
        .directive("sortableTrash", function () {

            /**
             * dog fucked hack for drag remove feature
             */
            return {
                restrict: "E",
                template: "<ul ng-sortable><li ng-repeat='item in items track by $index'></li></ul>",
                link    : function (scope) {
                    scope.items = [];
