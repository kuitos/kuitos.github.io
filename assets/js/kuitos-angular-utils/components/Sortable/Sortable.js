...
(function (factory) {
	...
	var dragEl,
		...
		/** @const */
        TRASH_ELEMENT_NODE_NAME = 'SORTABLE-TRASH',
		RSPACE = /\s+/g,
	...
	function Sortable(el, options) {
        ...
        if (isFirefox && !bind) {
            _on(document, 'dragover', this._onDocumentDragOver);
            bind = true;
        }
	...
	Sortable.prototype = /** @lends Sortable.prototype */ {
		...
		_onDrag: _throttle(function (evt) {
			...
			var rect = lastContainer.getBoundingClientRect(),
                ...
                x = evt.clientX || pointerX,
				y = evt.clientY || pointerY;

			// when drag out from container
            if (right < x || x < left || bottom < y || y < top) {
        ...
        _onDragOut: function (container, dragEl) {
            if (container[expando].options.group.pull !== "clone") {
                ...
                if (!trashElement) {
                    trashElement = document.querySelector((TRASH_ELEMENT_NODE_NAME + " ul").toLowerCase());
                }

                if (!trashElement.contains(dragEl)) {
        ...
        _onDocumentDragOver: function (evt) {
            pointerX = evt.clientX;
            pointerY = evt.clientY;
            if (evt.preventDefault !== void 0) {
                evt.preventDefault();
                !this.options.dragoverBubble && evt.stopPropagation();
            }
        },
			...
			if (evt) {
				...
				if (dragEl) {
					...
					if (rootEl !== dragEl.parentNode) {
                        ...
                        // when drag out remove
                        if (trashElement && trashElement.contains(dragEl.parentNode)) {
                            _dispatchEvent(null, rootEl, 'dragOutRemove', dragEl, rootEl, oldIndex, newIndex);
                        }
		...
		destroy: function () {
            ...
            _off(document, 'dragover', this._onDocumentDragOver);
